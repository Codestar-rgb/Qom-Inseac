...
extends Item {
        ...
        for (int dx = -3; dx <= 3; ++dx) {
            for (int dy = -3; dy <= 3; ++dy) {
                for (int dz = -3; dz <= 3; ++dz) {
                    ...
                    if (!"srparasites".equals(ad) || !ap.startsWith("harleskinn") && !ap.startsWith("poland_skin_block") || (lip = ItemInfestedBonemeal.stateOf(RL_LIPOMA, 0)) == null || !ItemInfestedBonemeal.canPlaceClean(world, p, lip) || !world.func_180501_a(p, lip, 18)) continue;
                    ItemInfestedBonemeal.smoke(world, p);
                    any = true;
                }
            }
    ...
    public EnumActionResult func_180614_a(EntityPlayer player, World world, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (clickedBlock == SRPBlocks.ParasiteSapling) {
            IGrowable growable;
            int hook = ForgeEventFactory.onApplyBonemeal((EntityPlayer)player, (World)world, (BlockPos)pos, (IBlockState)clickedState, (ItemStack)stack, (EnumHand)hand);
            if (hook > 0) {
                ItemInfestedBonemeal.spawnSaplingUseParticles(world, pos, world.field_73012_v);
