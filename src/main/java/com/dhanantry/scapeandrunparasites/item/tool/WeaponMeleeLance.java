...
public class WeaponMeleeLance extends WeaponToolMeleeBase {
   public WeaponMeleeLance(ToolMaterial material, String name, double attackspeed, float range, float attackD, boolean fear, byte id) {
   ...
   @Override
   public Item getNext() {
      return this == SRPItems.weapon_lance ? SRPItems.weapon_lanceSentient : null;
   }
   ...
   public boolean func_77644_a(ItemStack stack, EntityLivingBase target, EntityLivingBase attacker) {
      ...
      if (flag) {
         ...
         if (attacker.field_70170_p.field_73012_v.nextDouble() < chance) {
            SRPPotions.applyStackPotion(SRPPotions.DLER_E, target, 200, amp);
         }
      }
