...
public class SRPWorldEntitySpawner {
    ...
    private static String getSpawnFailureReason(EntityLiving.SpawnPlacementType spawnPlacementTypeIn, World worldIn, BlockPos pos) {
        ...
        if (!SRPWorldEntitySpawner.isValidEmptySpawnBlock(worldIn.func_180495_p(pos.func_177984_a()))) {
            ...
            block6: for (int k2 = 0; k2 < 3; ++k2) {
                ...
                for (int i4 = 0; i4 < l3; ++i4) {
                    ...
                    if (ground == EntityLiving.SpawnPlacementType.IN_AIR) {
                        ...
                        SRPWorldEntitySpawner.adjustAirSpawnPos((World)worldServerIn, blockpos$mutableblockpos);
                        if (!beforeAdjust.equals((Object)blockpos$mutableblockpos)) {
                            SRPWorldEntitySpawner.debugSpawn("VANILLA adjusted air spawn pos from " + SRPWorldEntitySpawner.posToString(beforeAdjust) + " to " + SRPWorldEntitySpawner.posToString((BlockPos)blockpos$mutableblockpos) + " for " + biome$spawnlistentry.field_76300_b.getSimpleName());
                        }
                    } else if (ground == EntityLiving.SpawnPlacementType.IN_WATER) {
                        beforeAdjust = blockpos$mutableblockpos.func_185334_h();
                        SRPWorldEntitySpawner.adjustWaterSpawnPos((World)worldServerIn, blockpos$mutableblockpos);
                        if (!beforeAdjust.equals((Object)blockpos$mutableblockpos)) {
                            SRPWorldEntitySpawner.debugSpawn("VANILLA adjusted water spawn pos from " + SRPWorldEntitySpawner.posToString(beforeAdjust) + " to " + SRPWorldEntitySpawner.posToString((BlockPos)blockpos$mutableblockpos) + " for " + biome$spawnlistentry.field_76300_b.getSimpleName());
                        }
                    } else if (ground == EntityLiving.SpawnPlacementType.ON_GROUND) {
                        beforeAdjust = blockpos$mutableblockpos.func_185334_h();
                        SRPWorldEntitySpawner.adjustGroundSpawnPos((World)worldServerIn, blockpos$mutableblockpos);
                        if (!beforeAdjust.equals((Object)blockpos$mutableblockpos)) {
                            SRPWorldEntitySpawner.debugSpawn("VANILLA adjusted ground spawn pos from " + SRPWorldEntitySpawner.posToString(beforeAdjust) + " to " + SRPWorldEntitySpawner.posToString((BlockPos)blockpos$mutableblockpos) + " for " + biome$spawnlistentry.field_76300_b.getSimpleName());
                        }
                    }
                    String failureReason = SRPWorldEntitySpawner.getSpawnFailureReason(ground, (World)worldServerIn, (BlockPos)blockpos$mutableblockpos);
                    if (failureReason != null) {
                        SRPWorldEntitySpawner.debugSpawn("VANILLA rejected location for " + biome$spawnlistentry.field_76300_b.getSimpleName() + " at " + SRPWorldEntitySpawner.posToString((BlockPos)blockpos$mutableblockpos) + " | reason=" + failureReason);
                        continue;
                    }
                    try {
                        ...
                        SRPWorldEntitySpawner.debugSpawn("VANILLA failed to create entity instance for " + biome$spawnlistentry.field_76300_b.getName() + " at " + SRPWorldEntitySpawner.posToString((BlockPos)blockpos$mutableblockpos) + " | reason=" + exception.getClass().getSimpleName() + ": " + exception.getMessage());
                        return j4;
                    }
                    ...
                    if (canSpawn == Event.Result.DENY || canSpawn == Event.Result.DENY && !bypassForgeDenyForFloating) {
                        SRPWorldEntitySpawner.debugSpawn("VANILLA denied by Forge canEntitySpawn for " + entityliving.func_70005_c_() + " at " + SRPWorldEntitySpawner.posToString((BlockPos)blockpos$mutableblockpos));
                        ...
                        if (entityliving.func_70058_J()) {
                            ...
                            SRPWorldEntitySpawner.applyDebugGlow((EntityLiving)entityliving);
                            SRPWorldEntitySpawner.debugSpawn("VANILLA spawned " + entityliving.func_70005_c_() + " at " + SRPWorldEntitySpawner.posToString(entityliving.func_180425_c()) + " | packCount=" + ++j2 + " | glow=" + SRPConfigSystems.debugSpawner);
                        } else {
                            SRPWorldEntitySpawner.debugSpawn("VANILLA failed collision check for " + entityliving.func_70005_c_() + " at " + SRPWorldEntitySpawner.posToString((BlockPos)blockpos$mutableblockpos));
                            entityliving.func_70106_y();
                        }
                        if (j2 >= ForgeEventFactory.getMaxSpawnPackSize((EntityLiving)entityliving)) {
                            SRPWorldEntitySpawner.debugSpawn("VANILLA reached max pack size for " + entityliving.func_70005_c_() + " at " + SRPWorldEntitySpawner.posToString((BlockPos)blockpos$mutableblockpos));
                            continue block5;
                        }
                    } else {
                        SRPWorldEntitySpawner.debugSpawn("VANILLA getCanSpawnHere returned false | selected=" + biome$spawnlistentry.field_76300_b.getName() + " | actualClass=" + entityliving.getClass().getName() + " | actualName=" + entityliving.func_70005_c_() + " | pos=" + SRPWorldEntitySpawner.posToString((BlockPos)blockpos$mutableblockpos));
                    }
                    j4 += j2;
                }
            }
    ...
    private static void adjustGroundSpawnPos(World worldIn, BlockPos.MutableBlockPos pos) {
        BlockPos adjusted = SRPWorldEntitySpawner.findGroundSpawnPos(worldIn, pos.func_185334_h());
        pos.func_189533_g((Vec3i)adjusted);
    }

    private static void adjustAirSpawnPos(World worldIn, BlockPos.MutableBlockPos pos) {
        BlockPos adjusted = SRPWorldEntitySpawner.findAirSpawnPos(worldIn, pos.func_185334_h());
        pos.func_189533_g((Vec3i)adjusted);
    }
    ...
    public static int findChunksForSpawningOrigin(WorldServer worldServerIn, boolean spawnHostileMobs, boolean spawnPeacefulMobs, boolean spawnOnSetTickRate) {
        ...
        eligibleChunksForSpawning.clear();
        SRPWorldData worldData = SRPWorldData.get((World)worldServerIn);
        SRPSaveData saveData225 = SRPSaveData.get((World)worldServerIn, 225);
        SRPSaveData saveData72 = SRPSaveData.get((World)worldServerIn, 72);
        ...
        block5: for (ChunkPos chunkpos1 : shuffled) {
            ...
            block6: for (int k2 = 0; k2 < 3; ++k2) {
                ...
                for (int i4 = 0; i4 < l3; ++i4) {
                    ...
                    EntityLiving.SpawnPlacementType ground = EntitySpawnPlacementRegistry.func_180109_a((Class)biome$spawnlistentry.field_76300_b);
                    if (ground == EntityLiving.SpawnPlacementType.IN_AIR) {
                        beforeAdjust = SRPConfigSystems.debugSpawner ? blockpos$mutableblockpos.func_185334_h() : null;
                        SRPWorldEntitySpawner.adjustAirSpawnPos((World)worldServerIn, blockpos$mutableblockpos);
                        if (beforeAdjust != null && !beforeAdjust.equals((Object)blockpos$mutableblockpos)) {
                            SRPWorldEntitySpawner.debugSpawn("ORIGIN adjusted air spawn pos from " + SRPWorldEntitySpawner.posToString(beforeAdjust) + " to " + SRPWorldEntitySpawner.posToString((BlockPos)blockpos$mutableblockpos) + " for " + biome$spawnlistentry.field_76300_b.getSimpleName());
                        }
                    } else if (ground == EntityLiving.SpawnPlacementType.IN_WATER) {
                        beforeAdjust = SRPConfigSystems.debugSpawner ? blockpos$mutableblockpos.func_185334_h() : null;
                        SRPWorldEntitySpawner.adjustWaterSpawnPos((World)worldServerIn, blockpos$mutableblockpos);
                        if (beforeAdjust != null && !beforeAdjust.equals((Object)blockpos$mutableblockpos)) {
                            SRPWorldEntitySpawner.debugSpawn("ORIGIN adjusted water spawn pos from " + SRPWorldEntitySpawner.posToString(beforeAdjust) + " to " + SRPWorldEntitySpawner.posToString((BlockPos)blockpos$mutableblockpos) + " for " + biome$spawnlistentry.field_76300_b.getSimpleName());
                        }
                    } else if (ground == EntityLiving.SpawnPlacementType.ON_GROUND) {
                        beforeAdjust = SRPConfigSystems.debugSpawner ? blockpos$mutableblockpos.func_185334_h() : null;
                        SRPWorldEntitySpawner.adjustGroundSpawnPos((World)worldServerIn, blockpos$mutableblockpos);
                        if (beforeAdjust != null && !beforeAdjust.equals((Object)blockpos$mutableblockpos)) {
                            SRPWorldEntitySpawner.debugSpawn("ORIGIN adjusted ground spawn pos from " + SRPWorldEntitySpawner.posToString(beforeAdjust) + " to " + SRPWorldEntitySpawner.posToString((BlockPos)blockpos$mutableblockpos) + " for " + biome$spawnlistentry.field_76300_b.getSimpleName());
                        }
                    }
                    String failureReason = SRPWorldEntitySpawner.getSpawnFailureReason(ground, (World)worldServerIn, (BlockPos)blockpos$mutableblockpos);
                    if (failureReason != null) {
                        SRPWorldEntitySpawner.debugSpawn("ORIGIN rejected location for " + biome$spawnlistentry.field_76300_b.getSimpleName() + " at " + SRPWorldEntitySpawner.posToString((BlockPos)blockpos$mutableblockpos) + " | reason=" + failureReason);
                        continue;
                    }
                    boolean withinOrigin = SRPWorldEntitySpawner.isPosWithinOrigin((World)worldServerIn, (BlockPos)blockpos$mutableblockpos, worldData, originsExist, originlessAllowed);
                    if (!withinOrigin) {
                        SRPWorldEntitySpawner.debugSpawn("ORIGIN adjusted position outside origin for " + biome$spawnlistentry.field_76300_b.getSimpleName() + " at " + SRPWorldEntitySpawner.posToString((BlockPos)blockpos$mutableblockpos));
                        continue;
                    }
                    try {
                        entityliving = (EntityParasiteBase)biome$spawnlistentry.newInstance((World)worldServerIn);
                        ...
                        SRPWorldEntitySpawner.debugSpawn("ORIGIN failed to create entity instance for " + biome$spawnlistentry.field_76300_b.getName() + " at " + SRPWorldEntitySpawner.posToString((BlockPos)blockpos$mutableblockpos) + " | reason=" + exception.getClass().getSimpleName() + ": " + exception.getMessage());
                        return j4;
                    }
                    float spawnX = (float)blockpos$mutableblockpos.func_177958_n() + 0.5f;
                    float spawnY = blockpos$mutableblockpos.func_177956_o();
                    ...
                    if (canSpawn == Event.Result.DENY) {
                        SRPWorldEntitySpawner.debugSpawn("ORIGIN denied by Forge canEntitySpawn for " + entityliving.func_70005_c_() + " at " + SRPWorldEntitySpawner.posToString((BlockPos)blockpos$mutableblockpos));
                        ...
                        if (entityliving.func_70058_J()) {
                            worldServerIn.func_72838_d((Entity)entityliving);
                            SRPWorldEntitySpawner.applyDebugGlow((EntityLiving)entityliving);
                            SRPWorldEntitySpawner.debugSpawn("ORIGIN spawned " + entityliving.func_70005_c_() + " at " + SRPWorldEntitySpawner.posToString(entityliving.func_180425_c()) + " | packCount=" + ++j2 + " | glow=" + SRPConfigSystems.debugSpawner);
                        } else {
                            SRPWorldEntitySpawner.debugSpawn("ORIGIN failed collision check for " + entityliving.func_70005_c_() + " at " + SRPWorldEntitySpawner.posToString((BlockPos)blockpos$mutableblockpos));
                            entityliving.func_70106_y();
                        }
                        if (j2 >= ForgeEventFactory.getMaxSpawnPackSize((EntityLiving)entityliving)) {
                            SRPWorldEntitySpawner.debugSpawn("ORIGIN reached max pack size for " + entityliving.func_70005_c_() + " at " + SRPWorldEntitySpawner.posToString((BlockPos)blockpos$mutableblockpos));
                            continue block5;
                        }
                    } else if (!canSpawnHere) {
                        SRPWorldEntitySpawner.debugSpawn("ORIGIN getCanSpawnHere returned false | selected=" + biome$spawnlistentry.field_76300_b.getName() + " | actualClass=" + entityliving.getClass().getName() + " | actualName=" + entityliving.func_70005_c_() + " | pos=" + SRPWorldEntitySpawner.posToString((BlockPos)blockpos$mutableblockpos));
                    } else {
                        SRPWorldEntitySpawner.debugSpawn("ORIGIN spawn failed for " + entityliving.func_70005_c_() + " at " + SRPWorldEntitySpawner.posToString((BlockPos)blockpos$mutableblockpos) + " | reason=unknown DEFAULT rejection");
                    }
                    j4 += j2;
                }
            }
    ...
    private static void filterEligibleChunksForOrigin(SRPWorldData data, boolean originsExist, boolean originlessAllowed) {
        ...
        while (chunkIterator.hasNext()) {
            ...
            for (i = 0; i < originCount; ++i) {
                if (!SRPWorldEntitySpawner.chunkIntersectsRadius2D(chunkPos.field_77276_a, chunkPos.field_77275_b, originsX.get(i), originsZ.get(i), originsA.get(i))) continue;
                inRange = true;
                break;
            }
            if (!inRange) {
                for (i = 0; i < colonyCount; ++i) {
                    if (!SRPWorldEntitySpawner.chunkIntersectsRadius2D(chunkPos.field_77276_a, chunkPos.field_77275_b, coloniesX.get(i), coloniesZ.get(i), colonyRadii[i])) continue;
                    inRange = true;
                    break;
                }
            }
    ...
    public static Biome.SpawnListEntry getSpawnListEntryForTypeAt(WorldServer worldServerIn, BlockPos pos) {
        ...
        SRPSaveData dat = SRPSaveData.get((World)worldServerIn, 72);
        ...
        if (SRPConfigWorld.originActivated) {
            SRPWorldData data = SRPWorldData.get((World)worldServerIn);
            boolean originsExist = data != null && !data.getorigins("x").isEmpty();
            SRPSaveData saveData225 = SRPSaveData.get((World)worldServerIn, 225);
            boolean originlessAllowed = saveData225 != null && saveData225.getDeveLevel() >= SRPConfigSystems.deveOriginlessUse;
            return SRPWorldEntitySpawner.getSpawnListEntryForTypeAtOrigin(worldServerIn, pos, dat, data, originsExist, originlessAllowed);
        }
        List<Biome.SpawnListEntry> list = SRPSpawning.getSpawns((World)worldServerIn, id, dat.getEvolutionPhase(id), dat);
        ...
        if (chosen == null) {
            ...
            SRPWorldEntitySpawner.debugSpawn("Selected spawn entry " + chosen.field_76300_b.getSimpleName() + " in dim " + id + " at " + SRPWorldEntitySpawner.posToString(pos) + " | originActivated=false | phase=" + dat.getEvolutionPhase(id));
        }
        return chosen;
    }

    @Nullable
    private static Biome.SpawnListEntry getSpawnListEntryForTypeAtOrigin(WorldServer worldServerIn, BlockPos pos, SRPSaveData dat, SRPWorldData data, boolean originsExist, boolean originlessAllowed) {
        Biome.SpawnListEntry chosen;
        int id = worldServerIn.field_73011_w.getDimension();
        if (!SRPWorldEntitySpawner.isPosWithinOrigin((World)worldServerIn, pos, data, originsExist, originlessAllowed)) {
        ...
        if (phase == -1) {
            ...
            List<Biome.SpawnListEntry> list = SRPSpawning.getSpawns((World)worldServerIn, id, phase, dat);
            ...
            if (chosen2 == null) {
                ...
                SRPWorldEntitySpawner.debugSpawn("Selected spawn entry " + chosen2.field_76300_b.getSimpleName() + " in dim " + id + " at " + SRPWorldEntitySpawner.posToString(pos) + " | originActivated=true | phase=" + phase + " | outbreak=true");
            }
            return chosen2;
        }
        byte phaseToUse = originsExist ? phase : (byte)0;
        List<Biome.SpawnListEntry> list = SRPSpawning.getSpawns((World)worldServerIn, id, phaseToUse, dat);
        Biome.SpawnListEntry spawnListEntry = chosen = list != null && !list.isEmpty() ? (Biome.SpawnListEntry)WeightedRandom.func_76271_a((Random)worldServerIn.field_73012_v, list) : null;
        if (chosen == null) {
            ...
            SRPWorldEntitySpawner.debugSpawn("Selected spawn entry " + chosen.field_76300_b.getSimpleName() + " in dim " + id + " at " + SRPWorldEntitySpawner.posToString(pos) + " | originActivated=true | phase=" + phaseToUse + (originsExist ? " | originsPresent=true" : " | forcedPhase=0 | noOrigins=true"));
        }
        return chosen;
    }
    ...
    private static void adjustWaterSpawnPos(World worldIn, BlockPos.MutableBlockPos pos) {
        BlockPos adjusted = SRPWorldEntitySpawner.findWaterSpawnPos(worldIn, pos.func_185334_h());
        pos.func_189533_g((Vec3i)adjusted);
    }
