...
public class CommandExtremeSnow extends CommandBase {
   ...
   public void func_184881_a(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
      if (args.length < 1) {
         ...
         if (!world.field_72995_K) {
            ...
            float intensity = args.length >= 2 ? (float)func_175756_a(args[1], 0.0, 1.0) : data.getIntensity();
            boolean anywhere = args.length >= 3 ? func_175764_a(args[2], 0, 1) == 1 : data.isForceAnywhere();
            float windDeg = args.length >= 4 ? (float)func_175756_a(args[3], 0.0, 359.0) : data.getWindDeg();
            float windSpeed = args.length >= 5 ? (float)func_175756_a(args[4], 0.0, 1.0) : data.getWindSpeed();
            ...
            func_152373_a(
               ...
               this,
               String.format(
                  "Extreme Snow: %s (intensity=%.2f, anywhere=%s, windDeg=%.0f, windSpeed=%.2f)",
                  enabled ? "ON" : "OFF",
                  intensity,
                  anywhere,
                  windDeg,
