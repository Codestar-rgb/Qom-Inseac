...
public class ParasiteEventEntity {
        ...
        if (!entityin.field_70170_p.field_72995_K) {
            ...
            while (flag) {
                ...
                if (mobListG[index] != null) {
                    ...
                    if (entityC.length != 3) {
                        SRPMain.logger.error("Malformed string: " + mobListG[index].toString() + " in the beckon spawn pool configuration, safely exiting loop. Did you forget a semicolon?");
                        return false;
                    }
                    double chance = Double.parseDouble(entityC[1]);
                    EntityCanSummon father = (EntityCanSummon)((Object)entityin);
                    ...
                    if (!ParasiteEventEntity.getWorldBeckonSpawnLimit(entityin)) {
                        ...
                        if (entityin.func_70638_az() != null) {
                            ...
                            entityAlt.updateSTR();
                            SRPMain.network.sendToAll((IMessage)new SRPPacketParticle(entityin.field_70165_t, entityin.field_70163_u, entityin.field_70161_v, 0.5f, 0.5f, 10));
                        }
                    } else {
    ...
    public static void convertEntity(EntityLivingBase entityin, NBTTagCompound tags, boolean ignoreKey, String[] list) {
        ...
        if (SRPSaveData.get(entityin.field_70170_p, 105).getEvolutionPhase(entityin.field_71093_bK) >= SRPConfigSystems.evolutionFeralNoSim && ParasiteEventEntity.convertEntityFeral(entityin, tags, true, list)) {
            return;
        }
        if (tags.func_74764_b("srpcothimmunity")) {
            ...
            int n = goo = SRPConfigSystems.disloCOTHTiers ? dataLol.getCurrentCode(entityin.field_70170_p.field_73011_w.getDimension(), 1) : 0;
            if (goo != 0) {
            ...
            for (String s : list) {
                ...
                if (outOne instanceof EntityPInfected) {
                    ...
                    for (EntityLivingBase mob : moblist) {
                        if (!mob.func_70644_a(SRPPotions.COTH_E) || !(tags = mob.getEntityData()).func_74764_b("srpcothimmunity") || (key = tags.func_74762_e("srpcothimmunity")) != 1 || mob.func_70660_b(SRPPotions.COTH_E).func_76458_c() <= 1) continue;
                        tags.func_74768_a("srpcothimmunity", ++key);
                    }
    ...
    public static void spawnInsider(EntityLivingBase entity, World world, NBTTagCompound tags) {
        ...
        if (tags.func_74764_b("srpcothimmunity")) {
            ...
            if (SRPConfigSystems.generationUse) {
                out.func_70606_j(out.func_110143_aJ() * ParasiteEventEntity.getSimCOTHMod(SRPSaveData.get(world, 102), world));
            }
            int range = 1;
            ...
            for (int k2 = -1 * range; k2 <= 1 * range && SRPConfig.paraGore; ++k2) {
                for (int l2 = -1 * range; l2 <= 1 * range; ++l2) {
                    ...
                    if (block != Blocks.field_150350_a || blockDown == Blocks.field_150350_a || !world.func_180495_p(blockpos.func_177977_b()).func_185913_b() || blockDown == SRPBlocks.InfestedStain || out.field_70170_p.field_73012_v.nextInt(4) != 0) continue;
                    out.field_70170_p.func_175656_a(blockpos, SRPBlocks.goreSim.func_176223_P().func_177226_a(BlockGore.VARIANT, (Comparable)((Object)BlockGore.EnumType.FLAT)));
    ...
    public static void alertAllPlayerSer(World w, String message, int warning) {
        ...
        List playerEntityList = w.func_73046_m().func_184103_al().func_181057_v();
        SRPMain.network.sendToServer((IMessage)new SRPPacketMovingSound(warning));
        for (EntityPlayerMP entityPlayerMP : playerEntityList) {
            entityPlayerMP.func_145747_a((ITextComponent)new TextComponentString(message));
        }
    }
    ...
    public static void spawnBeckon(World world, DamageSource cause, EntityParasiteBase in) {
        ...
        if (SRPConfigSystems.rsEnabled) {
            ...
            int count = 0;
            for (Entity entity : serverList) {
                if (!(entity instanceof EntityPBeckon) || ++count <= SRPConfig.nexusVenkrolCap && !(in.func_70068_e(entity) < (double)(SRPConfig.nexusVenkrolDis * SRPConfig.nexusVenkrolDis))) continue;
                return;
            }
            SRPWorldData data = SRPWorldData.get(world);
    ...
    public static boolean disloNumber2(World world) {
        ...
        for (int i = 0; i < serverList.size(); ++i) {
            int atm;
            if (!(serverList.get(i) instanceof EntityLivingBase) || !((EntityLivingBase)serverList.get(i)).func_70644_a(SRPPotions.JUGG_E) || count > (atm = ((EntityLivingBase)serverList.get(i)).func_70660_b(SRPPotions.JUGG_E).func_76458_c())) continue;
            count = atm;
            in = (EntityLivingBase)serverList.get(i);
        }
        if (in != null && count >= SRPConfigSystems.disloSummonByDeathKilling) {
            Random rand = new Random();
            int count2 = SRPSaveData.get(world, 99).getCurrentCode(world.field_73011_w.getDimension(), 2);
