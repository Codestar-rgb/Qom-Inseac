...
public class SRPEventHandlerBus {
    ...
    public void soundTwo(PlayStreamingSourceEvent event) {
        SRPMain.logger.debug("phase: {} s: {} ticks: {}", (Object)clientCurrentEvoPhase, (Object)event.getName(), (Object)musicTimer);
        ...
        if (!(event.getName().contains("srparasites") || clientCurrentEvoPhase <= 0 && clientScent <= 0)) {
            ...
            SRPMain.logger.debug("added {}", (Object)event.getName());
        }
    }
    ...
    public static void resetSouncTicker(int in) {
        ...
        if (atm2 <= 0 || SRPConfig.musicMax <= SRPConfig.musicMin) {
            int n = SRPConfig.musicMax + 1000;
        }
        if (musicTimer < (goo = rand.nextInt(atm2) + SRPConfig.musicMin) && musicTimer > 0) {
            return;
        }
        musicTimer = goo;
    }
    ...
    public void soundThree(TickEvent.PlayerTickEvent event) {
        ...
        if (event.player.field_70173_aa % 20 == 0) {
            SRPMain.network.sendToServer((IMessage)new SRPPacketRequestEvoPhaseClient());
        }
        ...
        if (musicTimer <= 0 && this.musicToRemove.isEmpty() && clientCurrentEvoPhase > 0) {
            SRPEventHandlerBus.resetSouncTicker(0);
    ...
    public void cropGrow(BlockEvent.CropGrowEvent.Pre event) {
        ...
        byte phase = SRPSaveData.get(event.getWorld(), 79).getEvolutionPhase(event.getWorld().field_73011_w.getDimension());
    ...
    public void onLivingDeath(LivingDeathEvent event) {
        ...
        server.func_184103_al().func_148539_a((ITextComponent)new TextComponentTranslation("death.attack.srparasites.ricardo", new Object[]{player.func_145748_c_()}));
    }
    ...
    public void onBlockPlaced(BlockEvent.PlaceEvent e) {
        ...
        SRPEventHandlerBus.tryConvertSponge(e.getWorld(), e.getPos(), e.getPlacedBlock());
    }
            ...
            try {
                if (changed == deadBlood) {
                    for (EnumFacing f : EnumFacing.values()) {
                        ...
                        if (SRPEventHandlerBus.isSponge(st.func_177230_c())) {
                            SRPEventHandlerBus.tryConvertSponge(world, n, st);
                            continue;
                        }
                        if (SRPEventHandlerBus.isWater(st)) {
                            IBlockState stain = SRPEventHandlerBus.getParasiteStainState();
                            if (stain.func_177230_c() == Blocks.field_150350_a) continue;
                            world.func_180501_a(n, stain, 2);
                            continue;
                        }
                        if (!SRPEventHandlerBus.isLava(st) || (rubble = SRPEventHandlerBus.getParasiteRubbleState()).func_177230_c() == Blocks.field_150350_a) continue;
                ...
                if (SRPEventHandlerBus.isWater(changedState) || SRPEventHandlerBus.isLava(changedState)) {
                    ...
                    if (touchesDeadBlood) {
                        if (SRPEventHandlerBus.isWater(changedState)) {
                            ...
                            IBlockState rubble = SRPEventHandlerBus.getParasiteRubbleState();
    ...
    private static void tryConvertSponge(World world, BlockPos spongePos, IBlockState spongeState) {
        ...
        if (!SRPEventHandlerBus.isSponge(spongeState.func_177230_c())) {
            return;
        }
        Block deadBlood = SRPEventHandlerBus.getDeadBloodBlock();
    ...
    public void entityHeal(LivingHealEvent event) {
        ...
        if (SRPConfigSystems.useEvolution && SRPSaveData.get(entity.field_70170_p, 80).getEvolutionPhase(entity.field_70170_p.field_73011_w.getDimension()) >= SRPConfigSystems.evolutionNoParasiteHealing) {
    ...
    public void entityHurt(LivingHurtEvent event) {
        ...
        if (entity.func_70644_a(SRPPotions.VIRA_E) && SRPConfigSystems.viralEnable) {
            amp = entity.func_70660_b(SRPPotions.VIRA_E).func_76458_c() + 1;
            damage = event.getAmount();
            event.setAmount(damage + damage * (amp * SRPConfigSystems.viralAmount));
        }
        if (entity.func_70644_a(SRPPotions.OVERHEATING_E) && (event.getSource() == DamageSource.field_76370_b || event.getSource() == DamageSource.field_76372_a)) {
            amp = entity.func_70660_b(SRPPotions.OVERHEATING_E).func_76458_c() + 1;
            damage = event.getAmount();
            event.getSource().func_151518_m();
            event.setAmount(damage + damage * (amp * 1.0f));
        }
        if (event.getSource().func_76346_g() instanceof EntityLivingBase && (mob = (EntityLivingBase)event.getSource().func_76346_g()).func_70644_a(SRPPotions.MUSCLEOUT_E)) {
            float amp2 = (float)mob.func_70660_b(SRPPotions.MUSCLEOUT_E).func_76458_c() + 1.0f;
            event.setAmount(event.getAmount() * (SRPConfigSystems.muscleoutDamageOut * amp2));
        }
        if (entity instanceof EntityPlayer) {
    ...
    private float playerArmor(EntityPlayer player, float damageV, DamageSource source) {
        ...
        for (ItemStack itemstack : player.field_71071_by.field_70460_b) {
            ...
            red += (float)this.hasResistance(damage, resistanceS, resistanceI, lag, player.field_70170_p.field_73012_v, type) * (lag ? SRPConfig.sentientPointReduction : SRPConfig.livingPointReduction);
    ...
    public void playerFishing(ItemFishedEvent event) {
        if (!event.getEntity().field_70170_p.field_72995_K && SRPConfigSystems.useEvolution) {
            ...
            if (SRPSaveData.get(event.getEntity().field_70170_p, 81).getEvolutionPhase(event.getEntity().field_70170_p.field_73011_w.getDimension()) >= SRPConfigSystems.evolutionStopFishing) {
                event.setCanceled(true);
            }
        }
    }
    ...
    public void mobFear(LivingDamageEvent event) {
        ...
        if (event.getSource() == DamageSource.field_76379_h && SRPConfigSystems.fearFallDamage != 0.0f) {
            event.setAmount(event.getAmount() * (SRPConfigSystems.fearFallDamage * (float)amp));
        }
        ...
        if (SRPConfigSystems.fearUnfair) {
            if (!in.field_70122_E) {
                event.setAmount(event.getAmount() * (SRPConfigSystems.fearAirDamage * (float)amp));
            }
            return;
        }
    ...
    public void serverTick(TickEvent.ServerTickEvent event) {
        ...
        if (SRPConfigWorld.originWorldCheckDebugSpeed > 0) {
            this.counerW += SRPConfigWorld.originWorldCheckDebugSpeed;
        }
        --ParasiteEventWorld.disloCool;
        if (this.counerW > SRPConfig.dayTickValue && (SRPConfigWorld.nodesActivated || SRPConfigWorld.coloniesActivated || SRPConfigSystems.useEvolution || SRPConfigWorld.originActivated)) {
            MinecraftServer ser = FMLCommonHandler.instance().getMinecraftServerInstance();
            if (ser.field_71305_c.length > 0) {
                SRPSaveData dat = SRPSaveData.get((World)ser.field_71305_c[0], 89);
    ...
    public void worldTick(TickEvent.WorldTickEvent event) {
        ...
        if (!event.world.field_72995_K) {
            ...
            if (this.meteor > SRPConfigWorld.meteorTick) {
                SRPWorldData data;
                this.meteor = 0;
                if (event.world.field_73012_v.nextDouble() < SRPConfigWorld.meteorChance && SRPConfig.spawnDays <= (int)event.world.func_82737_E() && (data = SRPWorldData.get(event.world)).getTriggerMet() && SRPSaveData.get(event.world, -549).getEvolutionPhase(event.world.field_73011_w.getDimension()) >= 0) {
            ...
            if (this.counerW < 0 && (SRPConfigWorld.nodesActivated || SRPConfigWorld.coloniesActivated || SRPConfigSystems.useEvolution || SRPConfigWorld.originActivated)) {
                ...
                SRPMain.logger.info("[EIV DEBUG] worldTick is calling createRandomOrigin. dim={} time={} counerW={} worldsChecked={} originActivated={} min={} max={}", (Object)event.world.field_73011_w.getDimension(), (Object)event.world.func_82737_E(), (Object)this.counerW, this.worldsChecked, (Object)SRPConfigWorld.originActivated, (Object)SRPConfigWorld.originCreatingDistanceMin, (Object)SRPConfigWorld.originCreatingDistanceMax);
    ...
    public void setLoot(LivingDropsEvent event) {
        if (event.getEntityLiving() instanceof EntityParasiteBase) {
            ...
            if (mob.disloNumberEighteen && SRPSaveData.get(event.getEntityLiving().field_70170_p, 87).getCurrentCode(event.getEntityLiving().field_70170_p.field_73011_w.getDimension(), 18) > 0) {
                return;
            }
            ...
            if (((EntityLivingBase)event.getEntity()).func_70644_a(SRPPotions.COTH_E)) {
                ...
                if (SRPConfigSystems.useEvolution) {
                    ...
                    if (SRPSaveData.get(event.getEntity().field_70170_p, 86).getEvolutionPhase(event.getEntity().field_70170_p.field_73011_w.getDimension()) >= SRPConfigSystems.evolutionCothStopLoot && (tags2 = event.getEntity().getEntityData()).func_74764_b("srpcothimmunity") && (key2 = tags2.func_74762_e("srpcothimmunity")) != 0) {
                        event.setCanceled(true);
                    }
        ...
        if (event.phase == TickEvent.Phase.START) {
            ...
            if (thePlayer.field_70170_p.field_72995_K) {
                ...
                if (ParasiteEventWorld.canBiomeStillExist(thePlayer.field_70170_p, thePlayer.func_180425_c(), false) >= 1 || isParaBiome) {
                    BiomeParasiteBase biomeChecked = isParaBiome ? (BiomeParasiteBase)thePlayer.field_70170_p.func_180494_b(thePlayer.func_180425_c()) : SRPReference.getBiomeFromInt(ParasiteEventWorld.canBiomeStillExistType(thePlayer.field_70170_p, thePlayer.func_180425_c(), false));
                    ...
                    if (fog < SRPConfigWorld.biomeFogDensity) {
                        SRPMain.network.sendTo((IMessage)new SRPPacketFog(fog, fogRed, fogGreen, fogBlue), (EntityPlayerMP)thePlayer);
                    }
                } else if (fog > 0.0f) {
                ...
                this.heart = 0;
                if (isParaBiome && (age = (data = SRPWorldData.get(thePlayer.field_70170_p)).nearestHeartAge(thePlayer.func_180425_c(), true, 0)) > 0) {
