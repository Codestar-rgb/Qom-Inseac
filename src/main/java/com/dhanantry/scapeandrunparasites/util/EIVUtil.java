...
public class EIVUtil {
    public static void createRandomOrigin(World worldIn, int min, int max) {
        ...
        if (number == 0 ? worldIn.field_73012_v.nextInt(SRPConfigWorld.originCreatingRandZero) != 0 : worldIn.field_73012_v.nextInt(SRPConfigWorld.originCreatingRand) != 0) {
            return;
        }
        ...
        int key = data.setOrigin(worldIn, pos.func_177958_n(), pos.func_177956_o(), pos.func_177952_p(), SRPConfigWorld.originHealth, SRPConfigWorld.originRadius);
        ...
        SRPMain.logger.info("[EIV DEBUG] RANDOM_ORIGIN result. Player={} playerPos={} originPos={} dx={} dz={} horizontalDistance={} trueDistance={} min={} max={} health={} radius={} resultKey={} totalOrigins={}", new Object[]{player.func_70005_c_(), player.func_180425_c(), pos, dx, dz, String.format("%.2f", horizontalDistance), String.format("%.2f", trueDistance), min, max, SRPConfigWorld.originHealth, SRPConfigWorld.originRadius, key, data.getorigins("x").size()});
    }
}
