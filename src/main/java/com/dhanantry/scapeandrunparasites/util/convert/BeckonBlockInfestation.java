    ...
    public static void beckonInfestation(World worldIn, BlockPos pos, Random rand, int stage, boolean fromVenkrol) {
        ...
        if (fromVenkrol) {
            ...
            if (!moblist.isEmpty() && stage < 4) {
                ...
                for (EntityParasiteBase mob : moblist) {
                    ...
                    if (worldIn.func_175724_o(venPos.func_177984_a()) < (float)SRPConfigSystems.rsBlockLight / 15.0f) {
                        ++flagV;
                    }
                    if (worldIn.func_175642_b(EnumSkyBlock.BLOCK, venPos.func_177984_a()) >= SRPConfigSystems.rsBlockLight) continue;
                    ++flagV;
                }
                if (flagV < 2) {
                    return;
        ...
        if (stage >= 4) {
            ...
            if (SRPConfigSystems.useEvolution) {
                if (pivot != 0) {
                    SRPSaveData.get(worldIn, 75).setTotalKills(worldIn.field_73011_w.getDimension(), SRPConfigSystems.valueBlock * covertedBlocks * (SRPConfigSystems.pivotPointMultiplier * pivot), true, worldIn, true, 53);
                } else {
                    SRPSaveData.get(worldIn, 74).setTotalKills(worldIn.field_73011_w.getDimension(), SRPConfigSystems.valueBlock * covertedBlocks, true, worldIn, true, 54);
                }
            }
        }
    }
        ...
        for (int dir = 0; dir <= 5; ++dir) {
            ...
            if (stage <= 3) {
                ...
                if (stage <= 3 && BeckonBlockInfestation.isCobbleBlock(lookingBlock, lookingState)) {
                    ++convertedBlocks;
                    worldIn.func_180501_a(helper, SRPBlocks.InfestedCobblestone.func_176223_P(), 3);
                    continue;
                }
                if (BeckonBlockInfestation.isPathBlock(lookingBlock)) {
                    ++convertedBlocks;
                    worldIn.func_180501_a(helper, SRPBlocks.InfestedSand.func_176223_P(), 3);
                    continue;
                }
                if (BeckonBlockInfestation.isFenceBlock(lookingBlock)) {
                    ++convertedBlocks;
                    worldIn.func_180501_a(helper, SRPBlocks.InfestedFence.func_176223_P(), 3);
                    continue;
                }
                if (BeckonBlockInfestation.isAnyPlanks(lookingBlock, lookingState)) {
                    ++convertedBlocks;
                    worldIn.func_180501_a(helper, SRPBlocks.InfestedPlanks.func_176223_P(), 3);
                    continue;
                }
                if (BeckonBlockInfestation.isGlassBlock(lookingBlock, lookingState)) {
                    ++convertedBlocks;
                    worldIn.func_180501_a(helper, SRPBlocks.InfestedGlass.func_176223_P(), 3);
                    continue;
                }
                if (BeckonBlockInfestation.isGlassPaneBlock(lookingBlock)) {
                    ++convertedBlocks;
                    worldIn.func_180501_a(helper, SRPBlocks.INFESTED_GLASS_PANE.func_176223_P(), 3);
                    continue;
                }
                if (BeckonBlockInfestation.isFurnaceBlock(lookingBlock)) {
                    ++convertedBlocks;
                    worldIn.func_180501_a(helper, SRPBlocks.INFESTED_FURNACE.func_176223_P(), 3);
                    continue;
                }
                if (BeckonBlockInfestation.isStoneStairs(lookingBlock, lookingState)) {
                    dst = SRPBlocks.InfestedStoneStairs.func_176223_P();
                ...
                if (BeckonBlockInfestation.isWoodStairs(lookingBlock, lookingState)) {
                    dst = SRPBlocks.InfestedPlanksStairs.func_176223_P();
                    copy = BeckonBlockInfestation.copyCommonProps(lookingState, dst);
                    ++convertedBlocks;
                    worldIn.func_180501_a(helper, copy, 3);
                    continue;
                ...
                if (lookingBlock.isWood((IBlockAccess)worldIn, helper)) {
                    ++convertedBlocks;
                    worldIn.func_180501_a(helper, SRPBlocks.InfestedTrunk.func_176223_P(), 3);
                    continue;
                }
            }
            ...
            if (lookingBlock instanceof BlockBase) {
                if (lookingBlock == SRPBlocks.BiomeHeart || lookingBlock == SRPBlocks.ColonyHeart || BeckonBlockInfestation.isSrpInfestedBlock(lookingBlock)) continue;
