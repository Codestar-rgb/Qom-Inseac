...
public class TileEntityInfestationPurifier extends TileEntity implements ITickable {
   ...
   private static final ResourceLocation INFEST_REMAIN_RL = new ResourceLocation("srparasites", "infestremain");
      ...
      if (this.field_145850_b != null && !this.field_145850_b.field_72995_K && this.running) {
         if (this.tickGate++ % 1 == 0) {
            if (CACHED_INFEST_REMAIN == null) {
               CACHED_INFEST_REMAIN = (Block)Block.field_149771_c.func_82594_a(INFEST_REMAIN_RL);
            }

            int processed = 0;

            while (processed < 32 && !this.queue.isEmpty() && this.total < 131072) {
               BlockPos cur = this.queue.poll();
               if (cur == null) {
                  break;
               }
               ...
               if (this.visited.add(curKey)) {
                  IBlockState st = this.field_145850_b.func_180495_p(cur);
                  if (PurifyMappings.isSrp(st)) {
                     IBlockState vanilla = PurifyMappings.mapToVanillaState(st);
                     if (vanilla != null) {
                        IBlockState finalState = BlockInfestationPurifier.tryCopyCommonProps(st, vanilla);
                        if (this.field_145850_b.func_180501_a(cur, finalState, 3)) {
                           this.total++;
                           ...
                           if (this.field_145850_b instanceof WorldServer
                              && this.field_145850_b.func_72935_r()
                              && this.field_145850_b.func_175678_i(cur.func_177984_a())) {
                              double x = cur.func_177958_n() + 0.5;
                              double y = cur.func_177956_o() + 1.0;
                              double z = cur.func_177952_p() + 0.5;
                              SRPNetwork.CHANNEL
                                 .sendToAllAround(
                                    new MsgSpawnPureParticles(x, y, z, 8 + this.field_145850_b.field_73012_v.nextInt(4), 0),
                                    new TargetPoint(this.field_145850_b.field_73011_w.getDimension(), x, y, z, 64.0)
                                 );
                           }
                        }
                     }
                     ...
                     for (BlockPos p : BlockPos.func_177980_a(minN, maxN)) {
                        if (!p.equals(cur) && PurifyMappings.isSrp(this.field_145850_b.func_180495_p(p))) {
                           this.offer(p);
                        }
                     }
                     ...
                     for (BlockPos px : BlockPos.func_177980_a(min, max)) {
                        if (!px.equals(cur) && PurifyMappings.isSrp(this.field_145850_b.func_180495_p(px))) {
                           this.offer(px);
                        }
                     }
   ...
   private void notifyFinished() {
      if (this.field_145850_b instanceof WorldServer) {
         if (this.starter != null) {
            EntityPlayer p = this.field_145850_b.func_152378_a(this.starter);
            if (p != null) {
               p.func_146105_b(new TextComponentTranslation("message.srparasites.purifier_purified", new Object[]{this.total}), true);
            }
         }
      }
   }
