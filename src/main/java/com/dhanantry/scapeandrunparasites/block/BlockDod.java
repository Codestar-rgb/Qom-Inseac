...
public class BlockDod extends BlockBase implements ITileEntityProvider {
   ...
   private void pushAndHit(World worldIn, BlockPos hitPos, EntityPlayer player) {
      if (!player.field_70128_L) {
         ...
         double lookX = -MathHelper.func_76126_a(yawRad);
         double lookZ = MathHelper.func_76134_b(yawRad);
         ...
         long lastHit = player.getEntityData().func_74763_f("srp_dod_last_hit");
         if (now - lastHit >= 20L) {
            player.getEntityData().func_74772_a("srp_dod_last_hit", now);
