...
extends BlockBase {
    ...
    public static void SpreadBiome(World worldIn, BlockPos pos, int age, int type) {
        ...
        for (int x = pos.func_177958_n() - 4; x <= pos.func_177958_n() + 4; ++x) {
            for (int z = pos.func_177952_p() - 4; z <= pos.func_177952_p() + 4; ++z) {
                ...
                if (data.isInRangeOfHeart(convert, distance) == -1 || ParasiteEventEntity.checkName(worldIn.func_180494_b(convert).getRegistryName().toString(), SRPConfigWorld.biomeBlackList, SRPConfigWorld.biomeBlackListInverted) || (current = worldIn.func_180494_b(convert)) instanceof BiomeParasiteBase || (target = SRPReference.getBiomeFromInt(type)) != null && current == target) continue;
    ...
    public static void spreadBiomeBlockStain(World worldIn, BlockPos pos, Random rand) {
        ...
        if (covertedBlocks != 0 && SRPConfigSystems.useEvolution) {
            SRPSaveData.get(worldIn, 76).setTotalKills(worldIn.field_73011_w.getDimension(), SRPConfigSystems.valueBlock * covertedBlocks, true, worldIn, true, 20);
        }
    }
