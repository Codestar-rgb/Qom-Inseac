...
public class BlockFogNullifier extends BlockBase implements ITileEntityProvider {
   ...
   public boolean func_180639_a(World w, BlockPos p, IBlockState s, EntityPlayer pl, EnumHand h, EnumFacing f, float x, float y, float z) {
      return !w.field_72995_K ? this.attemptConsumeUseAndClear(w, p) : true;
   }
   ...
   private int clearConnectedFog(World w, BlockPos origin) {
      ...
      if (q.isEmpty()) {
         ...
         while (!q.isEmpty() && cleared < cap) {
            ...
            if (this.isParasiteFog(w, p)) {
               this.spawnDispelParticles(w, p, r);
               w.func_180501_a(p, Blocks.field_150350_a.func_176223_P(), 3);
               cleared++;

               for (EnumFacing fx : EnumFacing.field_82609_l) {
                  BlockPos n = p.func_177972_a(fx);
                  if (!seen.contains(n) && this.isParasiteFog(w, n)) {
                     seen.add(n);
                     q.addLast(n);
                  }
               }
   ...
   private void spawnDispelParticles(World w, BlockPos p, Random r) {
      for (int i = 0; i < 4; i++) {
         double x = p.func_177958_n() + 0.2 + r.nextDouble() * 0.6;
         double y = p.func_177956_o() + 0.2 + r.nextDouble() * 0.6;
         double z = p.func_177952_p() + 0.2 + r.nextDouble() * 0.6;
      ...
      if (r.nextFloat() < 0.3F) {
         ...
         for (int i = 0; i < extra; i++) {
            double x = p.func_177958_n() + r.nextDouble();
            double y = p.func_177956_o() + r.nextDouble();
            double z = p.func_177952_p() + r.nextDouble();
            w.func_175688_a(EnumParticleTypes.SMOKE_NORMAL, x, y, z, 0.0, 0.0, 0.0, new int[0]);
         }
      }
   }
   ...
   public void getDrops(NonNullList<ItemStack> drops, IBlockAccess acc, BlockPos pos, IBlockState state, int fortune) {
      if (acc instanceof World) {
         World w = (World)acc;
         TileEntity te = w.func_175625_s(pos);
         if (te instanceof TileEntityFogNullifier) {
            int uses = ((TileEntityFogNullifier)te).getUsesRemaining();
            if (uses > 0) {
               ItemStack stack = new ItemStack(this);
               setUsesOnStack(stack, uses);
               drops.add(stack);
            }
         }
      }
   ...
   private static int getUsesFromStack(ItemStack stack) {
      return stack.func_77942_o() && stack.func_77978_p().func_74764_b("UsesRemaining") ? stack.func_77978_p().func_74762_e("UsesRemaining") : -1;
   }
   ...
   private static void setUsesOnStack(ItemStack stack, int uses) {
      ...
      tag.func_74768_a("UsesRemaining", Math.max(0, uses));
