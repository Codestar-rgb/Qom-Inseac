            ...
            if (this.killC > SRPConfig.reinforcerNidusValue) {
                ...
                if (ccc == 0) {
                    ...
                    for (int x = 0; x < serverList.size(); ++x) {
                        if (!(serverList.get(x) instanceof EntityPDispatcher) || ++count <= SRPConfig.nexusDodCap && !(this.getDistanceSq((Entity)serverList.get(x)) < (double)(SRPConfig.nexusDodDis * SRPConfig.nexusDodDis))) continue;
                        if (SRPConfigSystems.useEvolution) {
                            SRPSaveData data = SRPSaveData.get(this.field_145850_b, 49);
                            data.setTotalKills(this.field_145850_b.field_73011_w.getDimension(), this.killC * SRPConfig.reinforcerNidusMult, true, this.field_145850_b, true, 45);
                        }
                        return;
                    }
                    if (SRPConfigSystems.useEvolution) {
                        ...
                        if (ParasiteEventEntity.getRSchance(this.field_145850_b) == 0.0) {
                            data.setTotalKills(this.field_145850_b.field_73011_w.getDimension(), this.killC * SRPConfig.reinforcerNidusMult, true, this.field_145850_b, true, 46);
