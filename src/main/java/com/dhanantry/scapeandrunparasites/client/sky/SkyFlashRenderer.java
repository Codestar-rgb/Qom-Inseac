...
public final class SkyFlashRenderer {
   ...
   private static final List<SkyFlashRenderer.Flash> FLASHES = new ArrayList<>();

   public static void trigger(int durationTicks, double span, double altitude) {
      trigger(durationTicks, span, altitude, -15.0F);
   }

   public static void trigger(int durationTicks, double span, double altitude, float bankDeg) {
      triggerOffset(durationTicks, span, altitude, bankDeg, 0.0, 0.0, 0.0);
   }

   public static void triggerOffset(
      int durationTicks, double span, double altitude, float bankDeg, double yawOffsetDeg, double forwardOffset, double lateralOffset
   ) {
      ...
      if (mc.field_71441_e != null && mc.field_71439_g != null) {
         ...
         Vec3d lookR = rotateYaw(look, yawOffsetDeg);
         Vec3d rightR = rotateYaw(right, yawOffsetDeg);
         ...
         FLASHES.add(new SkyFlashRenderer.Flash(mc.field_71441_e.func_82737_E(), durationTicks, start, end, bankDeg));
      }
   }
   ...
   public static void onRenderWorld(RenderWorldLastEvent e) {
      if (!FLASHES.isEmpty()) {
         ...
         if (mc.field_71441_e == null) {
            ...
            Iterator<SkyFlashRenderer.Flash> it = FLASHES.iterator();

            while (it.hasNext()) {
               SkyFlashRenderer.Flash f = it.next();
               double now = (float)mc.field_71441_e.func_82737_E() + pt;
               float t = (float)((now - f.startTick) / f.durationTicks);
               if (t >= 1.0F) {
                  it.remove();
               } else if (!(t < 0.0F)) {
                  Vec3d pos = lerp(f.start, f.end, t);
                  ...
                  drawRibbonQuad(pos, tangent, n, b, 0.0, halfLen, halfThick, alpha, camX, camY, camZ);
                  drawRibbonQuad(pos, tangent, n, b, 60.0, halfLen, halfThick, alpha, camX, camY, camZ);
                  drawRibbonQuad(pos, tangent, n, b, 120.0, halfLen, halfThick, alpha, camX, camY, camZ);
               }
            }
   ...
   private static void drawRibbonQuad(
      Vec3d pos, Vec3d tangent, Vec3d n, Vec3d b, double phiDeg, float halfLen, float halfThick, int alpha, double camX, double camY, double camZ
   ) {
      double r = Math.toRadians(phiDeg);
      double cs = Math.cos(r);
      double sn = Math.sin(r);
      Vec3d u = n.func_186678_a(cs).func_178787_e(b.func_186678_a(sn));
      Vec3d p0 = pos.func_178787_e(tangent.func_186678_a(-halfLen)).func_178787_e(u.func_186678_a(-halfThick));
      Vec3d p1 = pos.func_178787_e(tangent.func_186678_a(halfLen)).func_178787_e(u.func_186678_a(-halfThick));
      Vec3d p2 = pos.func_178787_e(tangent.func_186678_a(halfLen)).func_178787_e(u.func_186678_a(halfThick));
      Vec3d p3 = pos.func_178787_e(tangent.func_186678_a(-halfLen)).func_178787_e(u.func_186678_a(halfThick));
      ...
      buf.func_181662_b(p0.field_72450_a - camX, p0.field_72448_b - camY, p0.field_72449_c - camZ)
         ...
         .func_181669_b(255, 255, 255, alpha)
         .func_181675_d();
   ...
   private static Vec3d lerp(Vec3d a, Vec3d b, float t) {
      return a.func_186678_a(1.0F - t).func_178787_e(b.func_186678_a(t));
   }
