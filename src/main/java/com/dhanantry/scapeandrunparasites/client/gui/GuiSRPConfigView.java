...
extends GuiScreen {
    ...
    private List<SRPConfigList.Entry> buildEntries(Configuration cfg) {
        ArrayList<SRPConfigList.Entry> out = new ArrayList<SRPConfigList.Entry>();
        if (cfg == null) {
        ...
        for (String catName : cfg.getCategoryNames()) {
            ...
            for (Map.Entry kv : cat.entrySet()) {
                ...
                String value = GuiSRPConfigView.prettyValue(p);
    ...
    private static String prettyValue(Property p) {
        try {
            switch (p.getType()) {
                ...
                case INTEGER: {
                    if (p.isList()) {
                        return GuiSRPConfigView.intListToString(p.getIntList());
                    }
                    return String.valueOf(p.getInt());
                }
                case DOUBLE: {
                    if (p.isList()) {
                        return GuiSRPConfigView.doubleListToString(p.getDoubleList());
                    }
                    return String.valueOf(p.getDouble());
                }
            }
            if (p.isList()) {
                return GuiSRPConfigView.stringListToString(p.getStringList());
            }
            return p.getString();
        }
        catch (Exception e) {
    ...
    private List<SRPConfigList.Entry> filterEntries(List<SRPConfigList.Entry> src, String query) {
        ...
        if (q.isEmpty()) {
            return new ArrayList<SRPConfigList.Entry>(src);
        }
    ...
    public void func_73863_a(int mouseX, int mouseY, float partialTicks) {
        ...
        this.func_73732_a(this.field_146289_q, I18n.func_135052_a((String)"gui.srparasites.config.subtitle.readonly", (Object[])new Object[0]), this.field_146294_l / 2, 28, 0xAAAAAA);
        this.hoverTooltip = null;
        if (this.list != null) {
            this.list.func_148128_a(mouseX, mouseY, partialTicks);
        }
        if (this.searchField != null) {
            this.func_73731_b(this.field_146289_q, I18n.func_135052_a((String)"gui.srparasites.config.search", (Object[])new Object[0]), this.field_146294_l / 2 - 140, 34, 0xAAAAAA);
    ...
    public void applyEditedValue(SRPConfigList.Entry target, String newVal) {
        ...
        for (int i = 0; i < this.allEntries.size(); ++i) {
            ...
            if (e == null || e.header || !GuiSRPConfigView.safeEq(e.category, target.category) || !GuiSRPConfigView.safeEq(e.key, target.key)) continue;
