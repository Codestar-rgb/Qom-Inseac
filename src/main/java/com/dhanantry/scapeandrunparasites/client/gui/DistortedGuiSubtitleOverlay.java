...
public class DistortedGuiSubtitleOverlay extends GuiSubtitleOverlay {
   private final Minecraft client;
   private final List<DistortedGuiSubtitleOverlay.Subtitle> subtitles = Lists.newArrayList();
   ...
   public void func_184068_a(ScaledResolution resolution) {
      if (!this.enabled && this.client.field_71474_y.field_186717_N) {
         this.client.func_147118_V().func_184402_a(this);
         this.enabled = true;
      } else if (this.enabled && !this.client.field_71474_y.field_186717_N) {
         this.client.func_147118_V().func_184400_b(this);
         this.enabled = false;
      }
      ...
      if (this.enabled && !this.subtitles.isEmpty()) {
         ...
         Vec3d vec3d = new Vec3d(
            this.client.field_71439_g.field_70165_t,
            this.client.field_71439_g.field_70163_u + this.client.field_71439_g.func_70047_e(),
            this.client.field_71439_g.field_70161_v
         );
         Vec3d vec3d1 = new Vec3d(0.0, 0.0, -1.0)
            .func_178789_a(-this.client.field_71439_g.field_70125_A * (float) (Math.PI / 180.0))
            .func_178785_b(-this.client.field_71439_g.field_70177_z * (float) (Math.PI / 180.0));
         ...
         int i = 0;
         int j = 0;
         Iterator<DistortedGuiSubtitleOverlay.Subtitle> iterator = this.subtitles.iterator();

         while (iterator.hasNext()) {
            DistortedGuiSubtitleOverlay.Subtitle subtitle = iterator.next();
            if (subtitle.getStartTime() + 3000L <= Minecraft.func_71386_F()) {
               iterator.remove();
            } else {
               String measure = GuiDistortionHelper.shouldDistortSubtitles(this.client)
                  ? GuiDistortionHelper.jamText(subtitle.getString())
                  : subtitle.getString();
         ...
         j = j
            + this.client.field_71466_p.func_78256_a("<")
            + this.client.field_71466_p.func_78256_a(" ")
            + this.client.field_71466_p.func_78256_a(">")
            + this.client.field_71466_p.func_78256_a(" ");

         for (DistortedGuiSubtitleOverlay.Subtitle subtitle : this.subtitles) {
            ...
            int l1 = MathHelper.func_76128_c(MathHelper.func_151238_b(255.0, 75.0, (float)(Minecraft.func_71386_F() - subtitle.getStartTime()) / 3000.0F));
            int i2 = l1 << 16 | l1 << 8 | l1;
            GlStateManager.func_179094_E();
            GlStateManager.func_179109_b((float)resolution.func_78326_a() - l - 2.0F, (float)(resolution.func_78328_b() - 30) - i * (i1 + 1), 0.0F);
      ...
      if (accessor.func_188712_c() != null) {
         ...
         for (DistortedGuiSubtitleOverlay.Subtitle subtitle : this.subtitles) {
            if (subtitle.getString().equals(s)) {
               subtitle.refresh(new Vec3d(soundIn.func_147649_g(), soundIn.func_147654_h(), soundIn.func_147651_i()));
               return;
            }
         }

         this.subtitles.add(new DistortedGuiSubtitleOverlay.Subtitle(s, new Vec3d(soundIn.func_147649_g(), soundIn.func_147654_h(), soundIn.func_147651_i())));
      }
   }
