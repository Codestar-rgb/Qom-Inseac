...
public class DistortionGuiSwapHandler {
    ...
    private void trySwapIngameGui(Minecraft mc) {
        try {
            ...
            SRPMain.logger.error("Exception while swapping distorted GUI hooks.", (Throwable)e);
        }
    }
    ...
    private void trySwapChatGui(GuiIngame gui, Minecraft mc) {
        try {
            ...
            if (chatField == null) {
                GuiDistortionHelper.chatHookAvailable = false;
                SRPMain.logger.warn("Could not find chat GUI field on {}", (Object)gui.getClass().getName());
                return;
            }
            ...
            if (oldChat == null) {
                GuiDistortionHelper.chatHookAvailable = false;
                SRPMain.logger.warn("Chat GUI field was null on {}", (Object)gui.getClass().getName());
                return;
            }
            ...
            SRPMain.logger.info("Chat GUI swapped to {}", (Object)(current == null ? "null" : current.getClass().getName()));
        }
        catch (Exception e) {
            GuiDistortionHelper.chatHookAvailable = false;
            SRPMain.logger.warn("Failed to swap chat GUI.");
            SRPMain.logger.error("Exception while swapping distorted chat GUI.", (Throwable)e);
        }
    }
    ...
    private void trySwapSubtitleOverlay(GuiIngame newGui, Minecraft mc) {
        try {
            ...
            if (subtitleField == null) {
                GuiDistortionHelper.subtitleHookAvailable = false;
                SRPMain.logger.warn("Could not find subtitle overlay field in {}", (Object)newGui.getClass().getName());
                return;
            }
            subtitleField.setAccessible(true);
            this.removeFinalIfPresent(subtitleField);
            Object oldValue = subtitleField.get(newGui);
            SRPMain.logger.info("Found subtitle field: {} type={} old={}", (Object)subtitleField.getName(), (Object)subtitleField.getType().getName(), (Object)(oldValue == null ? "null" : oldValue.getClass().getName()));
            subtitleField.set(newGui, (Object)new DistortedGuiSubtitleOverlay(mc));
            Object newValue = subtitleField.get(newGui);
            SRPMain.logger.info("Subtitle overlay swapped to {}", (Object)(newValue == null ? "null" : newValue.getClass().getName()));
        }
        catch (Exception e) {
            GuiDistortionHelper.subtitleHookAvailable = false;
            SRPMain.logger.warn("Failed to swap subtitle overlay for distortion hook.");
            SRPMain.logger.error("Exception while swapping distorted subtitle overlay.", (Throwable)e);
        }
    }
