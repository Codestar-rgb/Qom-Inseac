...
public class StatsPage extends GuiScreen {
   ...
   private int rightW;
   private int rightH;
   private StatsPage.TierStat hoveredBarStat = null;
   private int rightScroll = 0;
   private final List<StatsPage.TierStat> stats = new ArrayList<>();
   ...
   public void func_73866_w_() {
      ...
      this.rightScroll = clamp(this.rightScroll, 0, this.getRightMaxScroll());
      ...
      this.field_146292_n
         .add(
            new GuiButton(
               ...
               80,
               20,
               GuiDistortionHelper.jamTextIfNeeded(I18n.func_135052_a("gui.back", new Object[0]), this.isJumbled)
            )
         );
   }
   ...
   public void func_146274_d() throws IOException {
      ...
      if (dwheel != 0) {
         ...
         if (this.isInRightPanel(dx, dy)) {
            if (dwheel < 0) {
               this.rightScroll = Math.min(this.getRightMaxScroll(), this.rightScroll + step);
            } else {
   ...
   public void func_73863_a(int mouseX, int mouseY, float partialTicks) {
      ...
      if (wasJumbled != this.isJumbled && !this.field_146292_n.isEmpty()) {
         ...
         back.field_146126_j = GuiDistortionHelper.jamTextIfNeeded(I18n.func_135052_a("gui.back", new Object[0]), this.isJumbled);
      }
      ...
      this.drawCenteredNoShadow(
         GuiDistortionHelper.jamTextIfNeeded(I18n.func_135052_a("bestiary.tab.stats", new Object[0]), this.isJumbled),
   ...
   private void drawLeftBarGraph(int mouseX, int mouseY) {
      ...
      this.field_146289_q
         .func_175065_a(
            GuiDistortionHelper.jamTextIfNeeded(I18n.func_135052_a("bestiary.stats.chart_title", new Object[0]), this.isJumbled),
            this.leftX,
            titleY,
            -14540254,
            false
         );
      func_73734_a(this.leftX, titleY + 10, this.leftX + Math.min(this.leftW - 6, 120), titleY + 11, -2007082412);
      int chartX = this.leftX + 4;
      int chartY = this.leftY + 16;
      int chartW = this.leftW - 10;
      int chartH = this.leftH - 20;
      func_73734_a(chartX, chartY, chartX + chartW, chartY + chartH, 268435456);
      int axisColor = 1717459540;
      func_73734_a(chartX, chartY + chartH - 1, chartX + chartW, chartY + chartH, axisColor);
      func_73734_a(chartX, chartY, chartX + 1, chartY + chartH, axisColor);
      int max = 1;
      if (!this.isJumbled) {
         for (StatsPage.TierStat stat : this.stats) {
            if (stat.kills > max) {
               max = stat.kills;
            }
         }
      ...
      StatsPage.TierStat hovered = null;

      for (int i = 0; i < this.stats.size(); i++) {
         StatsPage.TierStat statx = this.stats.get(i);
         int centerX = chartX + i * slotW + slotW / 2;
         int barX = centerX - barW / 2;
         int usableH = chartH - 4;
         int shownKills = GuiDistortionHelper.getDisplayValue(statx.kills, this.isJumbled, 100 + i);
         int barH;
         if (this.isJumbled) {
            float fakeRatio = GuiDistortionHelper.getDisplayRatio(statx.kills, max, true, 1000 + i);
            barH = Math.round(fakeRatio * usableH);
            barH = Math.max(1, Math.min(usableH, barH));
         } else {
            barH = (int)Math.round((double)shownKills / max * usableH);
            if (shownKills > 0) {
         ...
         func_73734_a(barX, chartY + chartH - 2 - usableH, barX + barW, chartY + chartH - 2, 134217728);
         func_73734_a(barX, barY, barX + barW, chartY + chartH - 2, statx.color);
         if (hoverDx >= barX && hoverDx < barX + barW && hoverDy >= chartY && hoverDy < chartY + chartH) {
            hovered = statx;
            func_73734_a(barX - 1, barY - 1, barX + barW + 1, chartY + chartH - 1, 1090519039);
         }
      }
   ...
   private void drawRightStatsPanel() {
      ...
      this.drawSectionTitle(
         GuiDistortionHelper.jamTextIfNeeded(I18n.func_135052_a("bestiary.stats.total_kills", new Object[0]), this.isJumbled), x, killsHeaderY, rowW
      );
      ...
      this.drawStatRow(
         ...
         rowW,
         -7699333,
         GuiDistortionHelper.jamTextIfNeeded(I18n.func_135052_a("bestiary.stats.all_parasites", new Object[0]), this.isJumbled),
         GuiDistortionHelper.getDisplayValue(grandTotal, this.isJumbled, 5000)
      );
      y += 18;

      for (int i = 0; i < this.stats.size(); i++) {
         StatsPage.TierStat stat = this.stats.get(i);
         this.drawStatRow(
            x,
            y,
            rowW,
            stat.color,
            GuiDistortionHelper.jamTextIfNeeded(stat.label, this.isJumbled),
            GuiDistortionHelper.getDisplayValue(stat.kills, this.isJumbled, 5100 + i)
         );
         y += 16;
      }
      ...
      this.drawSectionTitle(
         GuiDistortionHelper.jamTextIfNeeded(I18n.func_135052_a("bestiary.stats.combat_totals", new Object[0]), this.isJumbled), x, combatHeaderY, rowW
      );
      ...
      if (prog != null) {
         ...
         this.drawStatRow(
            ...
            rowW,
            -7699333,
            GuiDistortionHelper.jamTextIfNeeded(I18n.func_135052_a("bestiary.stats.damage_to_parasites", new Object[0]), this.isJumbled),
            GuiDistortionHelper.jamTextIfNeeded(dmgTo, this.isJumbled)
         );
         cy += 18;
         this.drawStatRow(
            x,
            ...
            rowW,
            -7699333,
            GuiDistortionHelper.jamTextIfNeeded(I18n.func_135052_a("bestiary.stats.damage_from_parasites", new Object[0]), this.isJumbled),
            GuiDistortionHelper.jamTextIfNeeded(dmgFrom, this.isJumbled)
         );
         cy += 16;
         this.drawStatRow(
            x,
            ...
            rowW,
            -7699333,
            GuiDistortionHelper.jamTextIfNeeded(I18n.func_135052_a("bestiary.stats.deaths_by_parasites", new Object[0]), this.isJumbled),
            GuiDistortionHelper.jamTextIfNeeded(deaths, this.isJumbled)
         );
      } else {
   ...
   private void drawScrollHints() {
      ...
      if (this.getRightMaxScroll() > 0) {
         func_73734_a(rightBarX, listY, rightBarX + 1, listY + listH, 857874978);
         ...
         func_73734_a(rightBarX, thumbY, rightBarX + 2, thumbY + thumbH, 1717986918);
      }
   }

   private void drawSectionTitle(String s, int x, int y, int w) {
      this.field_146289_q.func_175065_a(s, x, y, -14540254, false);
      func_73734_a(x, y + 10, x + Math.min(w - 6, 120), y + 11, -2007082412);
   }
   ...
   private void drawStatRow(int x, int y, int w, int color, String label, String valueText) {
      func_73734_a(x, y, x + w, y + 13, 402653184);
      func_73734_a(x, y, x + 4, y + 13, color);
      ...
      String plainLabel = TextFormatting.func_110646_a(safeLabel);
      ...
      this.field_146289_q.func_175065_a(shownLabel, x + 8, y + 3, textColor, false);
      this.field_146289_q.func_175065_a(val, x + w - valueWidth - 3, y + 3, color, false);
   }
   ...
   private void rebuildStats() {
      ...
      if (prog != null) {
         for (TileEntityRelayController.ScanRegistry.Tier tier : TileEntityRelayController.ScanRegistry.getAllTiers()) {
            ...
            String label = I18n.func_188566_a(tierKey) ? I18n.func_135052_a(tierKey, new Object[0]) : tier.getDisplayName();
            int color = this.getTierColor(tier.getIdLower());
            this.stats.add(new StatsPage.TierStat(label, color, total));
         }
      }
   }
   ...
   private void enableDesignScissor(int x, int y, int w, int h) {
      ...
      int sx = Math.round((this.screenPanelX + x * this.uiScale) * scaleFactor);
      int sy = Math.round((this.screenPanelY + y * this.uiScale) * scaleFactor);
      int sw = Math.round(w * this.uiScale * scaleFactor);
      int sh = Math.round(h * this.uiScale * scaleFactor);
      if (sw > 0 && sh > 0) {
         GL11.glEnable(3089);
         GL11.glScissor(sx, this.field_146297_k.field_71440_d - (sy + sh), sw, sh);
      }
   }

   private void drawCenteredNoShadow(String text, int x, int y, int color) {
      this.field_146289_q.func_175065_a(text, x - this.field_146289_q.func_78256_a(text) / 2, y, color, false);
   }
