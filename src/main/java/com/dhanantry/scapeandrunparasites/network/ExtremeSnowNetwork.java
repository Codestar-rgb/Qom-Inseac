...
public final class ExtremeSnowNetwork {
   ...
   public static void init() {
      int id = 0;
      CHANNEL.registerMessage(ExtremeSnowNetwork.S2CExtremeSnow.Handler.class, ExtremeSnowNetwork.S2CExtremeSnow.class, id++, Side.CLIENT);
   }
   ...
   public static void broadcast(WorldServer world, boolean en, float inten, boolean any, float windDeg, float windSpeed) {
      for (EntityPlayerMP p : world.func_73046_m().func_184103_al().func_181057_v()) {
         if (p.field_70170_p == world) {
            CHANNEL.sendTo(new ExtremeSnowNetwork.S2CExtremeSnow(en, inten, any, windDeg, windSpeed), p);
         }
      }
   }

   public static class S2CExtremeSnow implements IMessage {
      ...
      public static class Handler implements IMessageHandler<ExtremeSnowNetwork.S2CExtremeSnow, IMessage> {
         public IMessage onMessage(ExtremeSnowNetwork.S2CExtremeSnow msg, MessageContext ctx) {
