...
public class MsgRequestScan implements IMessage {
   ...
   public static class Handler implements IMessageHandler<MsgRequestScan, IMessage> {
      public IMessage onMessage(MsgRequestScan msg, MessageContext ctx) {
         ...
         world.func_152344_a(() -> {
            ...
            if (te instanceof TileEntityRelayController) {
               ...
               if (rc.canScan()) {
                  boolean ok = rc.performScan(player);
                  if (ok) {
                     rc.startCooldown();
                     started = true;
                  }
               ...
               SRPNetwork.CHANNEL.sendTo(new MsgScanCooldown(msg.pos, total, remaining, started), player);
            }
         });
         return null;
      }
