...
public class MsgSyncCelestialPhase implements IMessage {
   private int dim;
   private int phase;
   ...
   public MsgSyncCelestialPhase(int dim, int phase, long nightIndex, Set<String> active, Set<String> forced) {
      ...
      this.activeIds = active == null ? new String[0] : active.toArray(new String[0]);
      this.forcedIds = forced == null ? new String[0] : forced.toArray(new String[0]);
   }
   ...
   public void fromBytes(ByteBuf buf) {
      ...
      for (int i = 0; i < a; i++) {
         this.activeIds[i] = readString(buf);
      }
      ...
      for (int i = 0; i < f; i++) {
         this.forcedIds[i] = readString(buf);
      }
   }
   ...
   public static class Handler implements IMessageHandler<MsgSyncCelestialPhase, IMessage> {
      ...
      public IMessage onMessage(MsgSyncCelestialPhase msg, MessageContext ctx) {
         if (ctx.side != Side.CLIENT) {
            debug("received on non-client side, ignoring (dim=" + msg.dim + ", phase=" + msg.phase + ")");
            return null;
         } else {
            debug("RECEIVED on client: dim=" + msg.dim + " phase=" + msg.phase + " night=" + msg.nightIndex);
            Minecraft.func_71410_x()
               .func_152344_a(
                  () -> {
                     ...
                     debug(
                        "applying to CelestialPhaseClient: dim="
                           + msg.dim
                           + " phase="
                           + msg.phase
                           + " night="
