...
extends EntityParasiteBase {
    ...
    public void func_70636_d() {
        ...
        if (!this.field_70170_p.field_72995_K) {
            if (SPConfigSystems.disloGiveBodies && this.func_70089_S() && this.srpTicks == 10 && SPSaveData.get(this.field_70170_p, 40).getCurrentCode(this.field_70170_p.field_73011_w.getDimension(), 20) >= 1) {
    ...
    protected void dyingBurst(boolean fromDeath, int value) {
        ...
        if (i > 0 && this.timeSinceIgnited == 0) {
            this.func_184185_a(SPSounds.FLESH_GROW, 10.0f, 1.0f);
        }
    ...
    public boolean func_70652_k(@Nonnull Entity entityIn) {
        ...
        if (this.delay >= 3) {
            ...
            if (this.getaSize() >= meh.getaSize()) {
                ...
                meh.func_70106_y();
                this.func_184185_a(SPSounds.FLESH_EAT, 1.0f, 1.0f);
                this.settSize(0.3f);
                this.resetDelay();
    ...
    protected SoundEvent func_184639_G() {
        return SPSounds.FLESH_GROWL;
    }

    protected SoundEvent func_184601_bQ(DamageSource damageSourceIn) {
        return SPSounds.FLESH_HURT;
    }

    protected SoundEvent func_184615_bR() {
        return SPSounds.FLESH_DEATH;
    }
    ...
    private boolean merge(EntityParasiteBase entityin, int code, String name) {
        if (SPConfigSystems.mergeRandom) {
            entityin.particleStatus((byte)7);
            Random rand = new Random();
            int index = rand.nextInt(SPConfigSystems.mergeMobTable.length);
            boolean flag = true;
            int limit = 0;
            while (flag) {
                if (index >= SPConfigSystems.mergeMobTable.length) {
                    index = 0;
                    ++limit;
                }
                if (limit == 2) {
                ...
                if (SPConfigSystems.mergeMobTable[index] != null) {
                    String[] entityC = SPConfigSystems.mergeMobTable[index].split(";");
                    ParasiteSummon.spawnM(entityin, new String[]{entityC[0] + ";1;1"}, 7, true, name);
                    flag = false;
                    if (SPConfigSystems.useEvolution) {
                        SPWorldData data = SPWorldData.get(entityin.field_70170_p);
                        SPSaveData.get(entityin.field_70170_p, 100).setTotalKills(entityin.field_70170_p.field_73011_w.getDimension(), SPConfigSystems.valueMerge, true, entityin.field_70170_p, true, 40);
                    }
                    return true;
                }
                ++index;
            ...
            String[] entityC;
            for (int i = 0; i < SPConfigSystems.mergeMobTable.length; ++i) {
                int points;
                if (SPConfigSystems.mergeMobTable[i] == null || (points = Integer.parseInt((entityC = SPConfigSystems.mergeMobTable[i].split(";"))[1])) != code) continue;
                ParasiteSummon.spawnM(entityin, new String[]{entityC[0] + ";1;1"}, 7, true, name);
                if (SPConfigSystems.useEvolution) {
                    SPSaveData.get(entityin.field_70170_p, 96).setTotalKills(entityin.field_70170_p.field_73011_w.getDimension(), SPConfigSystems.valueMerge, true, entityin.field_70170_p, true, 41);
                }
                return true;
            }
            Random rand = new Random();
            ...
            while (flag) {
                ...
                if (SPConfigSystems.mergeMobTable[index] != null) {
                    entityC = SPConfigSystems.mergeMobTable[index].split(";");
                    ParasiteSummon.spawnM(entityin, new String[]{entityC[0] + ";1;1"}, 7, true, name);
                    flag = false;
                    if (SPConfigSystems.useEvolution) {
                        SPSaveData.get(entityin.field_70170_p, 95).setTotalKills(entityin.field_70170_p.field_73011_w.getDimension(), SPConfigSystems.valueMerge, true, entityin.field_70170_p, true, 42);
                    }
                    return true;
                }
                ++index;
