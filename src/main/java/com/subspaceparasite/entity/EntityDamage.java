                    ...
                    for (int k2 = -1 * BGrange; k2 <= BGrange; ++k2) {
                        for (int l2 = -1 * BGrange; l2 <= BGrange; ++l2) {
                            for (int j = 1 + offsetT; j <= BGheight + offsetT; ++j) {
                                ...
                                if (!((double)bHard <= 0.3 && bHard >= 0.0f) && !(block instanceof IMetaName) || block == SPBlocks.BiomeHeart || block == SPBlocks.ColonyHeart || block == SPBlocks.ParasiteRubbleDense || block == SPBlocks.ParasiteCanisterActive || block == Blocks.field_150350_a || !block.canEntityDestroy(iblockstate, (IBlockAccess)this.field_70170_p, blockpos, (Entity)this) || !ForgeEventFactory.onEntityDestroyBlock((EntityLivingBase)this.getCaster(), (BlockPos)blockpos, (IBlockState)iblockstate)) continue;
