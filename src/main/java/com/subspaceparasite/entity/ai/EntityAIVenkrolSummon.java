...
extends EntityAIBase {
    ...
    public EntityAIVenkrolSummon(EntityPStationaryArchitect venkrol, int limit, int cooldown, int stage, int CAV, double CAM) {
        ...
        this.flag = false;
        this.counted = false;
        if (!SPConfigSystems.useEvolution && !SPConfigSystems.rsIgnoreCooldownAtSpawn) {
            ...
            if (this.parentEntity.func_70685_l((Entity)target)) {
                ...
                if (this.attackTimer % 20 == 0 && this.attackTimer > 0) {
                    ...
                    if (parent.getActualParasites() < parent.getTotalParasites() && this.limit < this.sLimit) {
                        if (this.sStage == 4) {
                            if (ParasiteSummon.SummonM(this.parentEntity, SPAttributes.VENKROLSIV_MOBTABLEG, 12, target.field_70165_t, target.field_70163_u, target.field_70161_v, target, false)) {
                                if (this.flag) {
                            ...
                            ground = SPAttributes.VENKROL_MOBTABLEG;
                            air = null;
                            switch (this.sStage) {
                                case 2: {
                                    ground = SPAttributes.VENKROLSII_MOBTABLEG;
                                    air = SPAttributes.VENKROLSII_MOBTABLEA;
                                    break;
                                }
                                case 3: {
                                    ground = SPAttributes.VENKROLSIII_MOBTABLEG;
                                    air = SPAttributes.VENKROLSIII_MOBTABLEA;
                                }
                            }
                    ...
                    if (this.limit >= this.sLimit) {
                        if (this.flag) {
                            if (this.extraM > 0.0 && this.attackTimer > this.sLimit * 20) {
                                if (this.sStage == 4) {
                                    if (ParasiteSummon.SummonM(this.parentEntity, SPAttributes.VENKROLSIV_MOBTABLEG, 5, this.parentEntity.field_70165_t, this.parentEntity.field_70163_u, this.parentEntity.field_70161_v, this.parentEntity.func_70638_az(), false)) {
                                        this.extraM -= 1.0;
                                    }
                                } else {
                                    ground = SPAttributes.VENKROL_MOBTABLEG;
