...
import com.subspaceparasite.world.gen.feature.WorldGenParasiteNexusProtection1;
...
extends EntityPStationaryArchitect {
    ...
    public boolean storeLodo(EntityParasiteBase in, boolean upgrade) {
        if (in.getParasiteIDRegister() == 5 && upgrade) {
            this.mobNameId.add("subspaceparasite:rupter");
        ...
        if ((in.getParasiteIDRegister() == 43 || in.getParasiteIDRegister() == 39) && upgrade) {
            ...
            if (this.crude >= 6 && this.mobNameId != null) {
                ...
                for (int i = 0; i < this.mobNameId.size(); ++i) {
                    ...
                    this.mobNameId.add("subspaceparasite:heed");
                    break;
                }
            }
            return true;
    ...
    protected boolean storeMudo(EntityParasiteBase in, boolean upgrade) {
        if (in.getParasiteIDRegister() == 12 && upgrade) {
            this.mobNameId.add("subspaceparasite:mangler");
            in.particleStatus((byte)7);
            in.func_70106_y();
            return true;
        }
        ...
        for (int safe = 0; canAdd && safe < 10; ++safe) {
            switch (prim) {
                case 0: {
                    ...
                    this.mobNameId.add("subspaceparasite:pri_yelloweye");
                    canAdd = false;
                    break;
                }
                case 1: {
                    if (!SPConfigMobs.canraEnabled) break;
                    this.mobNameId.add("subspaceparasite:pri_summoner");
                    canAdd = false;
                    break;
                }
                case 2: {
                    if (!SPConfigMobs.zetmoEnabled) break;
                    this.mobNameId.add("subspaceparasite:pri_bolster");
                    canAdd = false;
                    break;
                }
                case 3: {
                    if (!SPConfigMobs.shycoEnabled) break;
                    this.mobNameId.add("subspaceparasite:pri_longarms");
                    canAdd = false;
                    break;
                }
                case 4: {
                    if (!SPConfigMobs.arachnidaEnabled) break;
                    this.mobNameId.add("subspaceparasite:pri_arachnida");
                    canAdd = false;
                    break;
                }
                case 5: {
                    if (!SPConfigMobs.noglaEnabled) break;
                    this.mobNameId.add("subspaceparasite:pri_reeker");
                    canAdd = false;
                    break;
                }
                case 6: {
                    if (!SPConfigMobs.hullEnabled) break;
                    this.mobNameId.add("subspaceparasite:pri_manducater");
                    canAdd = false;
                }
            }
            if (++prim < 7) continue;
