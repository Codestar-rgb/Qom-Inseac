...
public final class DispatcherStage4DetectedAdvancement {
    private static final ResourceLocation ADV_ID = new ResourceLocation("subspaceparasite", "qliphoth_overload");
    ...
    public static void register() {
        ...
        System.out.println("[SP][Qlip] listener registered");
    }
    ...
    public void onEntityJoinWorld(EntityJoinWorldEvent e) {
        ...
        for (EntityPlayerMP p : ws.func_73046_m().func_184103_al().func_181057_v()) {
            ...
            if (!granted) {
                System.out.println("[SP][Qlip] grant failed or already done for " + p.func_70005_c_());
                continue;
            }
            SoundEvent se = SPSounds.DISLO_28;
            if (se != null) {
                ...
                System.out.println("[SP][Qlip] played sound " + (se.getRegistryName() == null ? "<no id>" : se.getRegistryName().toString()) + " for " + p.func_70005_c_());
            } else {
                System.out.println("[SP][Qlip] DISLO_28 is null (not registered)");
            }
            SPNetwork.CHANNEL.sendTo((IMessage)new MsgQlipShake(0, 20, true, true, 4.0f), p);
            System.out.println("[SP][Qlip] sent shake packet to " + p.func_70005_c_());
        }
    }
}
