...
extends WorldSavedData {
    ...
    private static void writePointCsv(World worldIn, int dimID, int srcID, String srcName, boolean plus, int requestedAmount, int processedAmount, int appliedDelta, int before, int after, int phaseBefore, int phaseAfter, boolean canChangePhase, boolean isFromSleep, String result) {
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(file, true));){
            ...
            SP_LOG.warn("[SP EP DEBUG] Failed to write point CSV for dim {} src {}.", (Object)dimID, (Object)srcName, (Object)e);
        }
    }
    ...
    public void addUpdateNumber(int in) {
        if (SPConfigWorld.originActivated) {
            for (int i = 0; i < this.dimEPid.size(); ++i) {
                ...
                long newSize = size;
                newHealth = (long)((double)newHealth + (double)size * (SPConfigWorld.originDailyHealth + SPWorldData.getPhaseHealth(this.dimEPevolution.get(i).byteValue())));
                newSize = (long)((double)newSize + (double)size * (SPConfigWorld.originDailySize + SPWorldData.getPhaseSize(this.dimEPevolution.get(i).byteValue())));
                ...
                if (!SPSaveData.debugPointCsv()) {
                    SP_LOG.debug("[SP EP DEBUG] VECTOR_DAY dim={} phase={} eivHealth={} eivArea={}", (Object)this.dimEPid.get(i), (Object)this.dimEPevolution.get(i), (Object)this.dimEIVHealth.get(i), (Object)this.dimEIVArea.get(i));
                }
                long points = this.dimEPevolution.get(i) == -1 ? (long)((int)((double)this.dimEIVHealth.get(i).intValue() * SPConfigWorld.originDailyEPPointsOutBreak)) : (long)((int)((double)this.dimEIVHealth.get(i).intValue() * SPConfigWorld.originDailyEPPoints));
                int pointToAdd = points > Integer.MAX_VALUE ? Integer.MAX_VALUE : (int)points;
                pointToAdd = Math.min(pointToAdd, SPCommandEvolution.getVectorPointCap(this.dimEPevolution.get(i).byteValue()));
        ...
        for (int i = 0; i < this.dimEPid.size(); ++i) {
            ...
            if (plus) {
                ...
                if (SPConfigSystems.debugEvolutionPointsConsole) {
                    SP_LOG.debug("[SP EP DEBUG] calculated points dim={} src={} before={} change={} after={} phase={} canChangePhase={} sleepBypass={}", (Object)dimID, (Object)srcID, (Object)beforePoints, (Object)in, (Object)kills, (Object)phase, (Object)canChangePhase, (Object)isFromSleep);
                }
                if (top && kills < 0) {
