...
public final class SPFogHandler {
    public static void register() {
        MinecraftForge.EVENT_BUS.register((Object)new SPFogHandler());
    }

    private boolean shouldApply() {
        return SPFogManager.get().isEnabled();
    }

    @SubscribeEvent
    public void onFogColors(EntityViewRenderEvent.FogColors e) {
        if (!SPFogManager.get().isEnabled()) {
            return;
        }
        float r = e.getRed();
        float g = e.getGreen();
