...
public final class BreatheShaderManager {
    private static final Logger LOG = LogManager.getLogger((String)"SP-Shader");
    ...
    public void enableFor(int ticks) {
        ...
        LOG.info("[SP Breathe] enableFor({})", (Object)ticks);
    }

    public void disable() {
        this.wantEnabled = false;
        this.ticksRemaining = 0;
        LOG.info("[SP Breathe] disable()");
    }
    ...
    public void onClientTick(TickEvent.ClientTickEvent e) {
        ...
        if (!this.wantEnabled) {
            if (current != null) {
                er.func_181022_b();
                this.applied = false;
                LOG.info("[SP Breathe] stopUseShader()");
            }
            if (this.registered) {
        ...
        if (!this.applied || current == null) {
            try {
                LOG.info("[SP Breathe] loadShader {}", (Object)POST);
                ...
                if (sg2 != null) {
                    ...
                    this.lastH = h;
                    this.applied = true;
                    LOG.info("[SP Breathe] loaded {}x{}", (Object)w, (Object)h);
                } else {
                    LOG.error("[SP Breathe] ShaderGroup is null after loadShader");
                    this.applied = false;
                }
            }
            catch (Throwable t) {
                this.nextRetryMs = System.currentTimeMillis() + 2000L;
                LOG.error("[SP Breathe] loadShader failed", t);
                this.applied = false;
            }
            return;
        }
