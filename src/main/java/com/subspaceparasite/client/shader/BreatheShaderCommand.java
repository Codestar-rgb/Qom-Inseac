...
implements IClientCommand {
    ...
    public void func_184881_a(MinecraftServer server, ICommandSender sender, String[] args) {
        ...
        if (enable) {
            ...
            sender.func_145747_a((ITextComponent)new TextComponentString("[SP] Breathe shader: ON"));
        } else {
            BreatheShaderManager.get().disable();
            SPFogManager.get().disable();
            sender.func_145747_a((ITextComponent)new TextComponentString("[SP] Breathe shader: OFF"));
        }
    }
