...
import com.subspaceparasite.client.EscapeClientState;
...
public class GuiGameOverEscape
extends GuiGameOver {
    ...
    public void func_73866_w_() {
        ...
        if (EscapeClientState.OFFER) {
            int min = Math.max(0, SPConfigWorld.escapeMinDistance);
            int max = Math.max(min, SPConfigWorld.escapeMaxDistance);
            String label = I18n.func_74838_a((String)"gui.subspaceparasite.escape_button") + min + "-" + max + " " + I18n.func_74838_a((String)"gui.subspaceparasite.escape_button_blocks");
    ...
    protected void func_146284_a(GuiButton button) throws IOException {
        if (button.field_146127_k == 911) {
            SPNetwork.CHANNEL.sendToServer((IMessage)new C2SRequestEscape());
    ...
    public static class OpenHook {
        ...
        public void onOpen(GuiOpenEvent e) {
            if (e.getGui() instanceof GuiGameOver && EscapeClientState.OFFER && !(e.getGui() instanceof GuiGameOverEscape)) {
                e.setGui((GuiScreen)new GuiGameOverEscape());
            }
        }
    }
}
