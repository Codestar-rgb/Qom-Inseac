...
extends SPEffectBase {
    ...
    private void effectNeedler(EntityLivingBase entity, int amplifier) {
        if (amplifier >= SPConfigSystems.needlerTerminal) {
            String mobname;
            amplifier -= SPConfigSystems.needlerTerminal;
            entity.func_184596_c(SPPotions.DLER_E);
            try {
                mobname = EntityList.func_191301_a((Entity)entity).toString();
            }
            catch (Exception e) {
                SPMain.logger.log(Level.ERROR, "Problem with needler and an entity", (Throwable)e);
                return;
            }
            if (ParasiteEventEntity.checkName(mobname, SPConfigSystems.needlerImmuneList, SPConfigSystems.needlerImmuneListWhite)) {
                return;
            }
            entity.func_70690_d(new PotionEffect(SPPotions.DLER_E, 400, amplifier, false, false));
            float f1 = entity.func_110143_aJ();
            if (f1 <= 0.0f) {
                return;
            }
            float damagee = entity.func_110138_aP() * SPConfigSystems.needlerDamage;
            damagee = entity instanceof EntityPlayer ? Math.min(damagee, SPConfigSystems.needlerMaxDamPlayer) : Math.min(damagee, SPConfigSystems.needlerMaxDamMonster);
