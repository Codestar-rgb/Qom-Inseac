...
extends BlockParasiteSpreading {
    ...
    public void func_180658_a(World worldIn, BlockPos pos, Entity entityIn, float fallDistance) {
        ...
        if (!SPConfigWorld.bloodyIceBreakOnHardLanding) {
            return;
        }
        if ((double)fallDistance < SPConfigWorld.bloodyIceBreakFallDistance) {
            return;
        }
        int diameter = SPConfigWorld.bloodyIceBreakDiameter;
        if (diameter < 1) {
            diameter = 1;
        }
