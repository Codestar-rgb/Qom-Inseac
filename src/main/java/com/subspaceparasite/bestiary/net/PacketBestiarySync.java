...
import com.subspaceparasite.client.FieldGuideClientSettings;
import com.subspaceparasite.client.celestial.CelestialSkyRenderer;
...
implements IMessage {
    ...
    private static boolean hasFieldGuide(EntityPlayer p) {
        ...
        Item guide = SPItems.SP_FIELD_GUIDE;
                ...
                mc.func_152344_a(() -> {
                    ...
                    if (canMakeNoise && pendingUnlockMask != 0 && PacketBestiarySync.tryConsumeUnlockSoundSlot()) {
                        if ((pendingUnlockMask & 4) != 0) {
                            ...
                            mc.field_71439_g.func_184185_a(SPSounds.BOOK_UNLOCK_CELESTIAL, 1.0f, 1.0f);
                        } else if ((pendingUnlockMask & 2) != 0) {
                            pendingUnlockMask = pendingUnlockMask & 0xFFFFFFFD;
                            mc.field_71439_g.func_184185_a(SPSounds.BOOK_UNLOCK_ENTITY, 1.0f, 1.0f);
                        } else if ((pendingUnlockMask & 1) != 0) {
                            pendingUnlockMask = pendingUnlockMask & 0xFFFFFFFE;
                            mc.field_71439_g.func_184185_a(SPSounds.BOOK_UNLOCK_BLOCK, 1.0f, 1.0f);
                        }
                    }
                    Handler.refreshGui(mc);
                });
