...
implements IMessage {
    ...
    public SPPacketParticle() {
    }

    public SPPacketParticle(double xx, double yy, double zz, float width, float height, byte particleType) {
        this.x = xx;
        this.y = yy;
        this.z = zz;
        this.width = width;
    ...
    implements IMessageHandler<SPPacketParticle, IMessage> {
        public IMessage onMessage(SPPacketParticle message, MessageContext ctx) {
            FMLCommonHandler.instance().getWorldThread(ctx.netHandler).func_152344_a(() -> this.handle(message, ctx));
            return null;
        }

        private void handle(SPPacketParticle message, MessageContext ctx) {
            Random rand = new Random();
            switch (message.type) {
                case 1: {
                    this.spawnParticles(SPEnumParticle.GSPLASH, rand, message, 0, 0, 0);
                    break;
                }
                case 2: {
                    for (int i = 0; i <= 3; ++i) {
                        this.spawnParticles(SPEnumParticle.GCLOUD, rand, message, 0, 0, 0);
                    }
                    break;
                }
                case 3: {
                    for (int i = 0; i <= 5; ++i) {
                        this.spawnParticles(SPEnumParticle.RHAPPY, rand, message, 0, 0, 0);
                    }
                    break;
                }
                case 4: {
                    for (int i = 0; i <= 23; ++i) {
                        this.spawnParticles(SPEnumParticle.GCLOUD, rand, message, 125, 227, 118);
                    }
                    break;
                }
                case 5: {
                ...
                case 10: {
                    ...
                    for (i = 0; i <= 20; ++i) {
                        if (i % 5 != 0) continue;
                        this.spawnParticlesGore(message, rand, SPEnumParticle.GSPLASH, 0, -1, -1);
                    }
                    for (i = 0; i <= 20; ++i) {
                        if (i % 5 != 0) continue;
                        this.spawnParticles(SPEnumParticle.GCLOUD, rand, message, 127, 0, 0);
                    }
                    break;
                }
                case 11: {
        ...
        private void spawnParticles(SPEnumParticle particleType, Random rand, SPPacketParticle message, int r, int g, int b) {
        ...
        private void spawnParticlesGore(SPPacketParticle message, Random ran, SPEnumParticle particleType, int r, int g, int b) {
            this.spawnParticlesGore(message, ran, particleType, r, g, b, 1.0, 4.0);
        }

        private void spawnParticlesGore(SPPacketParticle message, Random ran, SPEnumParticle particleType, int r, int g, int b, double xF, double yF) {
            Random rand = new Random();
