...
extends CommandBase {
    ...
    public void func_184881_a(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        if (args.length == 0) {
            sender.func_145747_a(CommandSPHelp.tr("srphelp.header.warning", new Object[0]));
            sender.func_145747_a(CommandSPHelp.tr("srphelp.header.topics", new Object[0]));
            for (String t : TOPICS.keySet()) {
                sender.func_145747_a(CommandSPHelp.tr("srphelp.topic.item", t));
            }
            sender.func_145747_a(CommandSPHelp.tr("srphelp.footer.hint", new Object[0]));
            return;
        }
        String topic = args[0].toLowerCase(Locale.ROOT);
        if (!TOPICS.containsKey(topic)) {
            sender.func_145747_a(CommandSPHelp.tr("srphelp.error.unknown", topic));
            sender.func_145747_a(CommandSPHelp.tr("srphelp.header.topics_inline", String.join((CharSequence)", ", TOPICS.keySet())));
            return;
        }
        sender.func_145747_a(CommandSPHelp.tr("srphelp.header.warning", new Object[0]));
        sender.func_145747_a(CommandSPHelp.tr("srphelp.topic.header", topic));
        for (Entry e : TOPICS.get(topic)) {
            sender.func_145747_a(CommandSPHelp.tr(e.usageKey, new Object[0]));
            sender.func_145747_a(CommandSPHelp.tr(e.key, new Object[0]));
        }
    }
    ...
    public List<String> func_184883_a(MinecraftServer server, ICommandSender sender, String[] args, BlockPos pos) {
        if (args.length == 1) {
            return CommandSPHelp.func_175762_a((String[])args, new ArrayList<String>(TOPICS.keySet()));
        }
        return Collections.emptyList();
    }

    static {
        TOPICS.put("subspaceparasite", Arrays.asList(new Entry("srphelp.subspaceparasite.setgeneration.desc", "srphelp.subspaceparasite.setgeneration.usage"), new Entry("srphelp.subspaceparasite.getgeneration.desc", "srphelp.subspaceparasite.getgeneration.usage"), new Entry("srphelp.subspaceparasite.readconfigurationfile.desc", "srphelp.subspaceparasite.readconfigurationfile.usage"), new Entry("srphelp.subspaceparasite.toggle_dotiledrops.desc", "srphelp.subspaceparasite.toggle_dotiledrops.usage"), new Entry("srphelp.subspaceparasite.toggle_domobevolution.desc", "srphelp.subspaceparasite.toggle_domobevolution.usage"), new Entry("srphelp.subspaceparasite.resetdatafile.desc", "srphelp.subspaceparasite.resetdatafile.usage"), new Entry("srphelp.subspaceparasite.parasites.desc", "srphelp.subspaceparasite.parasites.usage")));
