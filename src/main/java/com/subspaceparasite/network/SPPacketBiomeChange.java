...
public class SPPacketBiomeChange
implements IMessage {
    private BlockPos blockPos;
    private boolean convertToParasite;
    private int type;

    public SPPacketBiomeChange() {
    }

    public SPPacketBiomeChange(BlockPos pos, boolean convert, int typeP) {
        this.blockPos = pos;
    ...
    implements IMessageHandler<SPPacketBiomeChange, IMessage> {
        public IMessage onMessage(SPPacketBiomeChange message, MessageContext ctx) {
            FMLCommonHandler.instance().getWorldThread(ctx.netHandler).func_152344_a(() -> this.handle(message, ctx));
            return null;
        }

        private void handle(SPPacketBiomeChange message, MessageContext ctx) {
            SPMain.proxy.spreadBiome(message.blockPos, message.convertToParasite, message.type);
        }
    }
}
